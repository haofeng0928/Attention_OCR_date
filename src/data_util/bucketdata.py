...
class BucketData(object):
    ...
    def flush_out(self, bucket_specs, valid_target_length=float('inf'), go_shift=1):
        ...
        res = dict(bucket_id=None, data=None, zero_paddings=None,
                   encoder_mask=None, decoder_inputs=None, target_weights=None)
        '''
        ...
        def get_bucket_id():
            for idx in range(0, len(bucket_specs)):
                if bucket_specs[idx][0] >= self.max_width / 4 - 1 and bucket_specs[idx][1] >= self.max_label_len:
                    return idx
            return None
        ...
        # ENCODER PART
        # 图片的宽度列表，同一个batch里的宽度相等
        res['data_len'] = [a.astype(np.int32) for a in np.array(self.data_len_list)]
        ...
        # (4,3,512)
        res['zero_paddings'] = np.zeros([len(self.data_list), padd_len, 512], dtype=np.float32)

        # （4,27）
        encoder_mask = np.concatenate((np.ones([len(self.data_list), real_len], dtype=np.float32),
                                       np.zeros([len(self.data_list), padd_len], dtype=np.float32)), axis=1)
        ...
        for l_idx in range(len(self.label_list)):
            ...
            if label_len <= decoder_input_len:
                self.label_list[l_idx] = np.concatenate((self.label_list[l_idx],
