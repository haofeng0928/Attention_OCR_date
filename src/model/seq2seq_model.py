...
# from debug import print_tensor,print_tensor_shape
...
class Seq2SeqModel(object):
    ...
    def __init__(self,
        ...
        def seq2seq_f(lstm_inputs, decoder_inputs, seq_length, do_decode):
            ...
            :param lstm_inputs: 是一個tensor的list,list大小爲27,tensor大小爲（4,512）
            :param decoder_inputs: 是一個tensor的list,list大小爲17,tensor大小爲（4）
            :param seq_length:
            :param do_decode:
            :return:
            """
            ...
            pre_encoder_inputs, output_state_fw, output_state_bw = \
                tf.contrib.rnn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, lstm_inputs,
                                                        initial_state_fw=None, initial_state_bw=None,
                                                        dtype=tf.float32, sequence_length=None, scope=None)

            # N: 17 - 46 - 83 - 149 - 231
            encoder_inputs = [e*f for e, f in zip(pre_encoder_inputs, encoder_masks[:seq_length])]  # (?, 1024) * N
            top_states = [array_ops.reshape(e, [-1, 1, num_hidden*2]) for e in encoder_inputs]  # (?, 1, 1024) * N
            attention_states = array_ops.concat(top_states, 1)  # (?, N, 1024)

            initial_state = tf.concat(axis=1, values=[output_state_fw, output_state_bw])  # (?, 2048)
        ...
        targets = [decoder_inputs[i + 1] for i in xrange(len(decoder_inputs) - 1)]  # 32
        ...
        if forward_only:
            self.outputs, self.losses, self.attention_weights_histories = model_with_buckets(
                encoder_inputs_tensor, decoder_inputs, targets, self.target_weights, buckets,
                lambda x, y, z: seq2seq_f(x, y, z, True), softmax_loss_function=softmax_loss_function)
        else:
            self.outputs, self.losses, self.attention_weights_histories = model_with_buckets(
                encoder_inputs_tensor, decoder_inputs, targets, self.target_weights, buckets,
                lambda x, y, z: seq2seq_f(x, y, z, False), softmax_loss_function=softmax_loss_function)
